...
<!-- This page has a table that does NOT like mobile support -->
<script>
...
Before you can make addons, <mark>You need permission to do it</mark> since we don't like people violating copyright so we have a very strict license. This also includes modifying the behavior of ScrapComputers in any way.
...
- `Level 2`: You would receive the popup every 30 seconds.
- `Level 3`: ScrapComputers would be **ENTIRELY** disabled
...
Each ban reason can result in a different outcome. Maybe you only would get a popup or maybe your game would crash when loading it in.
...
| Bad Reputation             | You have a very horrible reputation in the community.                              | 2 (Popup every 30s) | Yes        | No         |
...
### Please ask for permission before creating an addon!

You need permission in case you want to make an addon. You don't have to ask again if you get permission and want to make more add-ons.

### DO NOT bypass the ban system!

We will be honest here If you try bypassing our ban system, you're an actual asshole doing it because not only you somehow broke one of our rules, you're trying to do ban evasion! Isn't that a way to ruin your reputation and be hated by the community?

### No lying to the community!

Your claims are what your add-on does, They gotta be a claim that would be true. It's fine if a claim is a joke but if it's a false claim, you would be violating this rule!
...
Because it's illegal to violate a license or copyright. No need to explain it further.

### Your reputation DOES matter!

We do NOT want developers with horrible reputations to make add-ons with our mod. We want developers that the community likes! If you're canceled, well you're never going to be able to make add-ons for scrapcomputers then.

### Code and User Documentation quality matters!

Your code documentation MUST be good! You must make it so other developers (Intermediate specifically) can understand the code without having a headache over why this function is needed.
...
In every single interactable (including components), You must add this at the end of your code that is NOT in any function. This is needed for the ban system to work in case a banned addon gets loaded.
...
Your add-on must be something that does a lot of changes in the mod. Things like just "Display Extension Addon" aren't allowed since this is not impressive enough! Making a printer addon is allowed since it could be useful in cases and requires some effort put into it!
